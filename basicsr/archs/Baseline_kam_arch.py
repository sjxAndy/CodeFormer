...
class Baseline(nn.Module):
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], dw_expand=1, ffn_expand=2):
        ...
        for num in dec_blk_nums:
            ...
            chan = chan // 2
            self.kam_dict.append(kernel_attention(chan, chan))
            # self.fuse_convs_dict.append(Fuse_sft_block(chan, chan))
    ...
    def forward(self, inp, w=1):
        ...
        for decoder, up, enc_skip, kam_block in zip(self.decoders, self.ups, encs[::-1], self.kam_dict):
            x = up(x)
            x = kam_block(x, enc_skip)
            x = x + enc_skip
            # x = fuse_block(enc_skip, x, w)
            x = decoder(x)
...
class BaselineKAMLocal(Local_Base, Baseline):
