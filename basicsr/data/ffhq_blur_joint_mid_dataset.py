...
class FFHQBlurJointMidDataset(data.Dataset):

    def __init__(self, opt):
        super(FFHQBlurJointMidDataset, self).__init__()
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            if self.use_blur_input:
                ...
                for gt_path in self.paths:
                    self.blur_paths.append(osp.join(self.blur_folder, osp.basename(gt_path)[0:5] + '_deblur.jpg'))
    ...
    def __getitem__(self, index):
        ...
        if self.use_blur_input:
            ...
            (img_gt, img_in), status = augment([img_gt, img_in], hflip=self.opt['use_hflip'], rotation=False, return_status=True)
            img_in_save = img_in
        else:
            # random horizontal flip
            img_gt, status = augment(img_gt, hflip=self.opt['use_hflip'], rotation=False, return_status=True)
        
        img_mid_gt = cv2.resize(img_in, (512, 512), interpolation=cv2.INTER_LINEAR)
        ...
        img_mid_gt = img2tensor(img_mid_gt, bgr2rgb=True, float32=True)
        ...
        normalize(img_mid_gt, self.mean, self.std, inplace=True)

        return_dict = {'in': img_in, 'in_large_de': img_in_large, 'gt': img_gt, 'gt_path': gt_path, 'mid_gt': img_mid_gt}
