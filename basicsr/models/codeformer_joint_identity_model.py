...
class CodeFormerJointIdentityModel(SRModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('identity_opt'):
            self.cri_cosine = build_loss(train_opt['identity_opt']).to(self.device)
            # logger.info(f'cri cosine device: {self.cri_cosine.y}')
        
        if train_opt.get('network_identity'):
            self.use_identity = True
        else:
            self.use_identity = False
        
        if self.use_identity:
            if train_opt.get('identity2'):
                self.use_identity2 = True
            else:
                self.use_identity2 = False
        logger.info(f'use identity: {self.use_identity}')
        logger.info(f'use identity2: {self.use_identity2}')
        if self.use_identity:
            # define identity network
            self.network_identity = build_network(train_opt['network_identity'])
    ...
    def optimize_parameters(self, current_iter):
        ...
        if current_iter % small_per_n == 0:
            ...
            logits, lq_feat = self.net_g(self.input, code_only=True)
            large_de = True
        ...
        if current_iter % self.net_d_iters == 0 and current_iter > self.net_g_start_iter:
            ...
            if not large_de: # when large degradation don't need image-level loss
                ...
                # identity loss
                if self.use_identity and not self.use_identity2:
                    ...
                    l_identity = self.cri_pix(identity_out, identity_gt) * identity_weight
                    l_g_total += l_identity
                    loss_dict['l_identity'] = l_identity
                if self.use_identity2:
                    identity_weight = self.opt['train']['identity_weight']
                    # get gray images and resize
                    out_gray = self.gray_resize_for_identity(self.output)
                    gt_gray = self.gray_resize_for_identity(self.gt)
                    ...
                    # self.cri_cosine = self.cri_cosine.to(self.device)
                    # logger.info(f'cri cosine change device: {self.cri_cosine.y}, {self.device}')

                    l_identity = self.cri_cosine(identity_out, identity_gt) * identity_weight
                    l_g_total += l_identity
                    loss_dict['l_identity'] = l_identity
