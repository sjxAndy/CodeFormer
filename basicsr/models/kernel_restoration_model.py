...
class KernelRestorationModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(KernelRestorationModel, self).__init__(opt)
        ...
        kernel_path = self.opt['path'].get('pretrain_network_kernel', None)
        if kernel_path is not None:
            if isinstance(self.net_g, (DataParallel, DistributedDataParallel)):
                self.load_network(self.net_g.module.kernel_extra, kernel_path, self.opt['path'].get('strict_load_g', True), param_key=self.opt['path'].get('param_key', 'params'))
            else:
                self.load_network(self.net_g.kernel_extra, kernel_path, self.opt['path'].get('strict_load_g', True), param_key=self.opt['path'].get('param_key', 'params'))
    ...
    def setup_optimizers(self):
        ...
        for k, v in self.net_g.named_parameters():
            if v.requires_grad:
                ...
                logger.info(f'Params {k} will be optimized.')
