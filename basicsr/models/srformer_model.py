...
class SRFormerModel(SRModel):
    def feed_data(self, data):
        self.gt = data['gt'].to(self.device)
        self.mid_gt = data['mid_gt'].to(self.device)
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('mid_opt'):
            self.cri_mid = build_loss(train_opt['mid_opt']).to(self.device)
        else:
            self.cri_mid = None
        
        if train_opt.get('mid_perceptual_opt'):
            self.cri_mid_perceptual = build_loss(train_opt['mid_perceptual_opt']).to(self.device)
        else:
            self.cri_mid_perceptual = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_mid:
            self.output, self.mid = self.net_g(self.input, w=1.0, detach_16=True)
        else:
            self.output = self.net_g(self.input, w=1.0, detach_16=True)
        ...
        if current_iter > self.net_g_start_iter:
            ...
            if self.cri_mid:
                l_g_mid = self.cri_mid(self.mid, self.mid_gt)
                l_g_total += l_g_mid
                loss_dict['l_g_mid'] = l_g_mid

            if self.cri_mid_perceptual:
                l_g_mid_percep = self.cri_mid_perceptual(self.mid, self.mid_gt)
                l_g_total += l_g_mid_percep
                loss_dict['l_g_mid_percep'] = l_g_mid_percep
